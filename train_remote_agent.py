...
(see ``slime.rollout.remote_agent.adapter_service``): the agent's OpenAI calls
hit the adapter, which routes them to the sglang router and records
token-level data per session for loss computation.
